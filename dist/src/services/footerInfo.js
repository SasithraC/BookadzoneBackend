...
const footerInfo_1 = __importDefault(require("../repositories/footerInfo"));
...
class FooterInfoService {
    constructor() {
        this.commonService = new common_service_1.CommonService(footerinfo_1.FooterInfoModel);
    }
    ...
    async updateFooterInfo(id, data, file) {
        ...
        const result = await footerInfo_1.default.updateFooterInfo(id, updateData);
    ...
    async softDeleteFooterInfo(id) {
        ...
        if (error)
            throw new Error(error.message);
        return await footerInfo_1.default.softDeleteFooterInfo(id);
    }
    async toggleStatus(id) {
        const error = validationHelper_1.default.isValidObjectId(id, "id");
        if (error)
            throw new Error(error.message);
        return await footerInfo_1.default.toggleStatus(id);
    }
    async getAllTrashFooterInfos(page = 1, limit = 10, filter) {
        return await footerInfo_1.default.getAllTrashFooterInfos(page, limit, filter);
    }
    async restoreFooterInfo(id) {
        const error = validationHelper_1.default.isValidObjectId(id, "id");
        if (error)
            throw new Error(error.message);
        return await footerInfo_1.default.restoreFooterInfo(id);
    }
    async deleteFooterInfoPermanently(id) {
        const error = validationHelper_1.default.isValidObjectId(id, "id");
        if (error)
            throw new Error(error.message);
        return await footerInfo_1.default.deleteFooterInfoPermanently(id);
    }
}
