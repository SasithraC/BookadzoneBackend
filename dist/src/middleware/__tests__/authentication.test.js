...
describe("Authentication Middleware", () => {
    ...
    describe("JWT Verification", () => {
        ...
        it("should return 403 if JWT verification returns undefined", async () => {
            jsonwebtoken_1.default.verify.mockReturnValue(undefined);
            await authenticate(req, res, next);
            expect(res.statusCode).toBe(403);
            expect(res._getJSONData().message).toBe("Invalid or expired token");
        });
        it("should return 403 if JWT is expired", async () => {
            ...
            jsonwebtoken_1.default.verify.mockImplementation(() => { throw error; });
            await authenticate(req, res, next);
            expect(res.statusCode).toBe(403);
            expect(res._getJSONData().message).toBe("Invalid or expired token");
        });
        ...
        it("should log JWT verification errors", async () => {
            ...
            jsonwebtoken_1.default.verify.mockImplementation(() => { throw error; });
            await authenticate(req, res, next);
            expect(consoleErrorSpy).toHaveBeenCalledWith("JWT Verification Error:", "jwt malformed", "Error stack trace");
        });
    });
    ...
    describe("Error Handling", () => {
        ...
        it("should log authentication errors for outer catch", async () => {
            ...
            await authenticate(req, res, next);
            expect(consoleErrorSpy).toHaveBeenCalledWith("Authentication Error:", "Database connection failed", "Database error stack trace");
        });
