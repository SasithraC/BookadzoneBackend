...
const seedSubMenus = async () => {
    try {
        await subMenuModel_1.SubmenuModel.deleteMany();
        ...
        const submenuItems = [
            ...
            { menu_slug: "setting", name: "UserRole", slug: "settings-userrole", path: "/userrole", order: 10 },
            // Trash
        ...
        for (const item of submenuItems) {
            const mainMenu = await menuModel_1.MenuModel.findOne({ slug: item.menu_slug, isDeleted: false });
            if (!mainMenu) {
        ...
        const insertedSubmenus = await subMenuModel_1.SubmenuModel.insertMany(submenuData);
